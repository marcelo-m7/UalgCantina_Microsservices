...
DEFAULT_REDIRECT = "http://localhost:3000/token"
...
@app.get("/callback")
def google_callback(code: str, redirect: str = DEFAULT_REDIRECT):
    try:
        id_token = exchange_code_for_token(code)
        user_info = verify_google_token(id_token)
        jwt_token = create_internal_token(user_info)
        return RedirectResponse(f"{redirect}?token={quote(jwt_token)}")
    except HTTPException as e:
        return RedirectResponse(f"{redirect}?error={e.detail}")
